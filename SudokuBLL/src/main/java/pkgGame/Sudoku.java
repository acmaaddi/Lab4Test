...
public class Sudoku extends LatinSquare {
	...
	private HashMap<Integer, Cell> map = new HashMap<Integer, Cell>();
	...
	//private HashMap<Integer, Cell> cells= new HashMap<Integer,Cell>()
		//INSERT andrews stuff here
		
		
		//here should be:hasmap code, getallvalidvalues, and SET CELLS
	...
	public Sudoku(int iSize) throws Exception {
		...
		FillDiagonalRegions();
		Cell mycell= new Cell(0,3);
		
		setCells();
		fillRemaining(this.map.get(Objects.hash(0, 0)));
	}
	...
	public Sudoku(int[][] puzzle) throws Exception {
		...
		setCells();
		fillRemaining(this.map.get(Objects.hash(0, 0)));   //IDK WHY THIS DOESNT WORK
	...
	private HashSet<Integer>getAllValidCellValues(int iRow,int iCol){
		HashSet<Integer> badVals = new HashSet<Integer>();
		HashSet<Integer> goodVals = new HashSet<Integer>();
		int[][] puzzle1 = this.getPuzzle();
		
		
		for (int i =0; i<iSize; i++) {
			goodVals.add(i);
			//this adds all possible vales to goodvals and then i wanna subtract out the bad vals
			
		}
		for (int i =0; i<iSize; i++) {
			if (puzzle1[iRow][i] > 0) {
				badVals.add(puzzle1[iRow][i]);
				
			}
			if (puzzle1[i][iCol] > 0) {
				badVals.add(puzzle1[i][iCol]);
		...
		ArrayList<Integer> badValsLIST=  new ArrayList<Integer> (badVals);
		for (int i=0; i<badVals.size();i++) {
			
			if (goodVals.contains(badValsLIST.get(i))) {
				goodVals.remove(badValsLIST.get(i));
			}
	...
	//We struggled a lot with fillRemaining and tried our best to learn from what was shown to us in lecture when we wrote this.
	public boolean fillRemaining(Cell mycell) {

        if (mycell == null) return true;

        for (int num : mycell.getlstValidValues()) {

                       if (isValidValue(mycell.getiRow(),mycell.getiCol(), num)) {  

                                      this.getPuzzle()[mycell.getiRow()][mycell.getiCol()] = num;

                                      if (fillRemaining(mycell.GetNextCell(mycell))) return true;
...
private class Cell extends java.lang.Object{
	...
	public void setiCol(int tCol) {  //double check to make sure these setters are neede dor used
		tCol=iCol;
		
	}


	public void setiRow(int tRow) {
		tRow= iRow;
    ...
    public boolean equals (Object o) {
        ...
        Cell mycell = (Cell) o;
        return iCol == mycell.iCol && iRow == mycell.iRow;
    }
